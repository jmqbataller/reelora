...
export const REELORA_CAPABILITY_CATALOG: FeatureCapability[] = [
  ...
  cap("ai-generated-video-remix", "editing", "implemented", "Accept one or many uploaded generated videos and rebuild them into a polished Reel using only their existing frames."),
  cap("all-uploaded-video-coverage", "editing", "implemented", "Require every uploaded source video to contribute at least one shot; stop with a clear error instead of silently omitting an upload."),
  cap("balanced-multi-source-remix", "editing", "implemented", "Balance planned shots across uploaded source videos while preserving upload and chronological order in re-edit mode."),
  cap("source-usage-audit", "qa", "implemented", "Report shot count and planned duration for every uploaded source plus an all-uploaded-videos-used result."),
  cap("scene-isolated-remix-windows", "editing", "implemented", "Detect source scene boundaries, trim their setup/tail edges, and select distinct windows instead of passing through the original timeline."),
